...
def http_get(
    ...
    if response.status_code == 200:
        ...
        raise ConfluenceException(
            f"Error {response.status_code}: {response.reason} on requesting {request_url}",
        )
...
def write_html_2_file(path, title, content, html_template, additional_headers=None):
    ...
    for placeholder, replacement in list(replacements.items()):
        ...
        try:
            ...
            raise ConfluenceException(
                f"Error {e}: Cannot replace placeholders in template file.",
            ) from e
